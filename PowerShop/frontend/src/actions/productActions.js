...
export const listMyProducts = (pageNumber = '', category) => async (
  dispatch,
  getState
) => {
  try {
    ...
    const { data } = await axios.get(
      `/api/products/myproducts?pageNumber=${pageNumber}&category=${category}&email=${userInfo.email}`,
      config
    );
...
export const updateProductStatus = (productId, status) => async (
  dispatch,
  getState
) => {
  try {
    ...
    const { data } = await axios.put(
      `/api/products/update/status/${productId}`,
      { status },
      config
    );
